...
custom_output info "Configure .env & .credentials files..."
{
  ...
  echo "MAGENTO_ADMIN_PASSWORD=a111111"
...

{
  ...
  echo 'magento/MAGENTO_ADMIN_PASSWORD=a111111'
...
sudo --preserve-env=MAGENTO_DIR sh -c 'envsubst "${MAGENTO_DIR}" < "${ASSETS_DIR}/templates/nginx.conf.template" > /etc/nginx/http.d/default.conf'
sudo --preserve-env=USER,PHP_MEMORY_LIMIT sh -c 'envsubst < "${ASSETS_DIR}/templates/fpm-pool.conf.template" > /usr/local/etc/php-fpm.d/www.conf'
sudo php-fpm -D -R
sudo nginx
