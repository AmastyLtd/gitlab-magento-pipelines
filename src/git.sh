...
get_clone_branch() {
    ...
    if [ -n "$CI_COMMIT_BRANCH" ] && git ls-remote --exit-code --heads "$_remote_url" "$CI_COMMIT_BRANCH" >/dev/null 2>&1; then
      _result_branch="$CI_COMMIT_BRANCH"
    elif [ -n "$CI_MERGE_REQUEST_SOURCE_BRANCH_NAME" ] && git ls-remote --exit-code --heads "$_remote_url" "$CI_MERGE_REQUEST_SOURCE_BRANCH_NAME" >/dev/null 2>&1; then
      _result_branch="$CI_MERGE_REQUEST_SOURCE_BRANCH_NAME"
    elif [ -n "$CI_MERGE_REQUEST_TARGET_BRANCH_NAME" ] && git ls-remote --exit-code --heads "$_remote_url" "$CI_MERGE_REQUEST_TARGET_BRANCH_NAME" >/dev/null 2>&1; then
...
get_git_remote_url() {
    ...
    if echo "$1" | grep "@${CI_SERVER_HOST}" >/dev/null 2>&1; then
        echo "$1"
    else
